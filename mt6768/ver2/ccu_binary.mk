...
$(linked_module): PRIVATE_PM_MAX := $(my_ccu_pm_max_code)
$(linked_module): PRIVATE_DM_MAX := $(my_ccu_dm_max_code)
...
$(linked_module): $(all_objects) $(built_static_libraries)
	...
	@PMSIZE=$$($(LOAD_MD32_TOOLCHAIN) $(PRIVATE_SIZE) --common $< | grep -v text | awk '{printf $$1}'); DMSIZE=$$($(LOAD_MD32_TOOLCHAIN) $(PRIVATE_SIZE) --common $< | grep -v text | awk '{printf $$2}');echo $${PMSIZE} $${DMSIZE};if [ $${PMSIZE} -le $(PRIVATE_PM_MAX) -a $${DMSIZE} -le $(PRIVATE_DM_MAX) ]; then $(LOAD_MD32_TOOLCHAIN) $(PRIVATE_LD) -o $@ $(PRIVATE_ALL_OBJECTS) $(if $(strip $(PRIVATE_ALL_STATIC_LIBRARIES)),--start-group $(PRIVATE_ALL_STATIC_LIBRARIES) --end-group) $(PRIVATE_LDFLAGS); else touch $@; fi
