...
LOCAL_LDFLAGS1 += -Wl,--script=$(CCU_TOOL_CUSTFUNC_DIR)/custfunc.ld

LOCAL_CFLAGS += -I$(CCU_TOOL_CUSTFUNC_DIR)/include
LOCAL_CFLAGS += -I$(CCU_TOOL_CUSTFUNC_DIR)/include/custfunc
LOCAL_CFLAGS += -I$(CCU_TOOL_COMMON_DIR)/include
LOCAL_CFLAGS += -I$(MTK_PATH_SOURCE)/hardware/libcamera_3a/libccu_lib/lib3a.ccu.headers/$(TARGET_BOARD_PLATFORM)/include
...
$(linked_module): PRIVATE_LD := $(LOCAL_CC)
$(linked_module): PRIVATE_SIZE := $(LOCAL_SIZE)
$(linked_module): PRIVATE_MAX := $(MAX_SIZE_PM)
$(linked_module): PRIVATE_LDFLAGS := $(LOCAL_LDFLAGS) $(LOCAL_LDFLAGS1)
...
my_ccu_stripped_module1 := $(LOCAL_BUILT_MODULE).dm
$(my_ccu_stripped_module1): PRIVATE_OBJCOPY := $(LOCAL_OBJCOPY)
$(my_ccu_stripped_module1): PRIVATE_SIZE := $(LOCAL_SIZE)
$(my_ccu_stripped_module1): PRIVATE_LINK_MODULE := $(linked_module)
$(my_ccu_stripped_module1): $(linked_module)
	@echo "target Strip: $(PRIVATE_MODULE) 1 ($@)"
	@mkdir -p $(dir $@)
	@rm -f $<.pm $<.dm $@
	$(hide) $(PRIVATE_OBJCOPY) -O binary -j.data -j.sdata -j.sbss -j.bss -j.rodata $< $@

my_ccu_stripped_module2 := $(LOCAL_BUILT_MODULE).pm
$(my_ccu_stripped_module2): PRIVATE_OBJCOPY := $(LOCAL_OBJCOPY)
$(my_ccu_stripped_module2): PRIVATE_SIZE := $(LOCAL_SIZE)
$(my_ccu_stripped_module2): PRIVATE_LINK_MODULE := $(linked_module)
$(my_ccu_stripped_module2): $(linked_module)
	@echo "target Strip: $(PRIVATE_MODULE) 1 ($@)"
	@mkdir -p $(dir $@)
	@rm -f $<.pm $<.dm $@
	$(hide) $(PRIVATE_OBJCOPY) -O binary -j.text $< $@	

my_ccu_stripped_module_ddr := $(LOCAL_BUILT_MODULE).ddr
$(my_ccu_stripped_module_ddr): PRIVATE_OBJCOPY := $(LOCAL_OBJCOPY)
$(my_ccu_stripped_module_ddr): PRIVATE_SIZE := $(LOCAL_SIZE)
$(my_ccu_stripped_module_ddr): PRIVATE_LINK_MODULE := $(linked_module)
$(my_ccu_stripped_module_ddr): $(linked_module)
	@echo "target Strip: $(PRIVATE_MODULE) ddr ($@)"
	@mkdir -p $(dir $@)
	@rm -f $<.ddr $@
	$(hide) $(PRIVATE_OBJCOPY) -O binary -j.data -j.bss -j.text -j.rodata $< $@

my_ccu_expend_dm := $(my_ccu_stripped_module1).exp
$(my_ccu_expend_dm): $(my_ccu_stripped_module1)
	@DMSIZE=`cat $< | wc -c`; echo "DM1 $$DMSIZE";\
	RMSIZE=`expr $(MAX_SIZE_DM) - $$DMSIZE`; echo "DM1 $$RMSIZE";\
	dd if=/dev/zero of=$<.empty bs=$$RMSIZE count=1;
	@cat $< $<.empty > $<.exp

my_ccu_expend_pm := $(my_ccu_stripped_module2).exp
$(my_ccu_expend_pm): $(my_ccu_stripped_module2)
...
my_ccu_expend_all := $(my_ccu_stripped_module_ddr).exp
$(my_ccu_expend_all): $(my_ccu_stripped_module_ddr)
	@PMSIZE=`cat $< | wc -c`; echo "PM2 $$PMSIZE";\
	PRMSIZE=`expr $(MAX_SIZE_ALL) - $$PMSIZE`; echo "PM2 $$PRMSIZE";\
	dd if=/dev/zero of=$<.empty bs=$$PRMSIZE count=1;
	@cat $< $<.empty > $<.exp

$(LOCAL_BUILT_MODULE) : $(my_ccu_expend_all)
...
$(LOCAL_BUILT_MODULE): PRIVATE_PM := $(my_ccu_expend_all)
$(LOCAL_BUILT_MODULE): $(my_ccu_expend_all)
	$(hide) mkdir -p $(dir $@)
	@touch $@
	@cat $(PRIVATE_PM) > $@
